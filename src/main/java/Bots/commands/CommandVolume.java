...
public class CommandVolume extends BaseCommand {
    ...
    public void execute(CommandEvent event) {
        ...
        if (args.length == 1) {
            ...
            event.replyEmbeds(createQuickSuccess(localise("cmd.vol.defaulted")));
        } else {
            if (args[1].matches("[^\\d.]")) {
                event.replyEmbeds(createQuickError(localise("cmd.vol.incorrectArg")));
                return;
            }
            if (args[1].matches("^\\d+$")) {
                ...
                if (volume > 500) {
                    event.replyEmbeds(createQuickError(localise("cmd.vol.tooHigh")));
                    return;
                }
                if (volume < 0) {
                    event.replyEmbeds(createQuickError(localise("cmd.vol.tooLow")));
                    return;
                }
                musicManager.audioPlayer.setVolume(volume);
                event.replyEmbeds(createQuickSuccess(localise("cmd.vol.success", volume)));
            } else {
                //More specific error if they don't get the point from the [^\\d.] error above
                event.replyEmbeds(createQuickError(localise("cmd.vol.invalidValue")));
            }
        }
    }
